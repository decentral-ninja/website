...
- End-to-End encryption Part 1:
  ...
  - [x] a: Each user creates per room a private/public key pair. The private key is stored locally, the public key at the user object along the uid.
  - [x] b: A user can create synchronous keys (not shared plainly) but stored locally. These keys can be used at multiple rooms.
  - [x] c: (Private key message) A synchronous key is salted, encrypted with the public key of other/receiving user and shared as a message with public viewable properties: TO user uid and key uid.
  ------------
  Should read:
  ------------
  ...
  ---------------------------
    - [x] 1. Encrypted message: User [User A] salts and encrypts message with synchronous key (b). Message object holds key uid at public viewable properties.
    - [x] 2. Public request message: Other user [User B], who does not have the synchronous key locally (b), see's encrypted message with UI hint (UI-Request) to request the synchronous key. The user sends the request holding the key uid in a public request message to every one, on user interaction.
    - [x] 3. Private key message: Users [User B or other User] who have the requested key, see the public request message with the key uid request as UI hint (UI-Reply) to reply with the requested synchronous key (c) as a private key message holding public viewable properties TO user uid and key uid (c).
    - [x] 4. At requesting user [User A], the UI hint (UI-Request) on message (2) to request is removed, since key, received as private key message, was decrypted, stored locally and message (1) was decrypted.
    - [x] 5. For every user, the public request message UI hint (UI-Reply) is marked checked/done and disabled by the existence (:has(...)) of the reply as private key message.
  ...
  ---------------------
    - [x] 1. Private key message: User [User A] sends key in a message holding public viewable properties TO user uid and key uid (c).
    - [x] 2. Receiving user [User B] sees UI hint to store key locally
  ---------------------
    - [x] show KeyName at KeyStatus when active
    - [x] on reply choose same key if applicable
    - [x] on receive, set received key as default, if no active key
    - [x] display: none request messages should be hidden first and also no delete effect if user does not have key to send and display-none anyway
    - [x] generate truly random room name
    - [x] key sorting
- [x] cooler https://github.com/ipv6rslimited/cloudseeder/blob/main/LICENSE.md , AGPL-3.0 https://github.com/Chocobozzz/PeerTube?tab=AGPL-3.0-1-ov-file#readme
- [x] reload smart icon (move it left of notification icon)
  - [x] refresh after longer session inactive and focus comes back, check navigator.onLine, provider pings when all status "timeout" and mutually connected users
    - [x] mobile on focus ask by dialog to refresh (reload), when the last activity to provider is old: https://developer.mozilla.org/en-US/docs/Web/API/Page_Visibility_API
- [x] when room delete also delete localStorage and Indexeddb
- [x] provider: data.getWebsocketInfo "get-info"
  - [x] receives os (https://nodejs.org/docs/latest/api/os.html#os_os_cpus) cpus (calc free cpu "i'm using this code:" https://stackoverflow.com/questions/9565912/convert-the-output-of-os-cpus-in-node-js-to-percentage), freemem (avoid low value), ~~uptime (rank by uptime)~~. inform on low websocket performance/overload and hint to switch to other provider. ~~share new switched provider (optional) as message~~, disconnect overloaded provider workflow or at least hint the user to do this manually. Also, postpone rapid notification pulling, for this, check from time to time at providers.getData the getWebsocketInfo and inform through event the notification controller.
- [x] linter, ~~Update yjs rollup stuff~~ rollup stuff needs updates, note only yjs had major changes by 2026
- [x] Service Worker for caching assets PWA + Fallback local hosting von decentral.ninja, https://docs.pwabuilder.com/#/home/sw-intro?id=pre-caching-during-the-install-event
- [x] host on multiple strong websocket instances ([pangolin](https://pangolin.net/))
  - [x] test again webRTC server and fire up an instance, also get some backup server instances
  - [x] server.js pass new vapid keys by env var
  - [x] cleanup; fallbacks at environment and server.js... start new proper instances, not local tunnel 'Bypass-Tunnel-Reminder': 'yup'
  - [x] from local instance https://github.com/Weedshaker/y-websocket
  - [x] from local instance https://github.com/yjs/y-webrtc
...
- [x] **ipfs and webtorrent integrations (https://developer.mozilla.org/en-US/docs/Web/API/Navigator/registerProtocolHandler?retiredLocale=de)** with WakeLock for video playing | https://developer.mozilla.org/en-US/docs/Web/API/Media_Session_API | https://developer.mozilla.org/en-US/docs/Web/API/Picture-in-Picture_API + encrypt files with stream https://github.com/SocketDev/wormhole-crypto, https://github.com/xuset/indexeddb-chunk-store/tree/master, https://github.com/webtorrent/webtorrent/blob/master/docs/tutorials.md, consistent webtorrent: https://github.com/webtorrent/webtorrent/issues/1293, webtorrent stream: https://github.com/Chocobozzz/PeerTube/blob/develop/server/core/helpers/webtorrent.ts, https://developer.mozilla.org/en-US/docs/Web/API/File_System_API/Origin_private_file_system
  ...
  - [x] prompt if encryption is wanted for videos
  - [x] icon indicate file encryption
  - [x] client did reset + 1x manual torrent reset === header reload icon
  - [x] download decrypted file link
  - [x] checkbox pause/resume torrent
  - [x] pin file that it does not garbage collect when running out of OPFS space ~~(don't allow more pins from certain limit)~~
  - [x] auto garbage collect OPFS
...
- [ ] ipfs and webtorrent integrations Part 2:
  ...
  - [ ] choose if seeding all torrents in current room or all rooms
  - [ ] user can manually change opfs storage limit (opfsMobileQuota + opfsDesktopQuota)
  - [ ] OPFS delete Option per room, etc. general Storage management: https://developer.mozilla.org/en-US/docs/Web/API/Storage_API navigator.storage.estimate() or at least info on available storage
- [ ] Provider Part 2:
  - [ ] fix trystero not showing the connected users reliably
  - [ ] overload provider backup triangle warning hint click connect trystero
  - [ ] ipns key as provider
  - [ ] user p2p graph webrtc url string split "," multiple urls and show separate (seen at history users dialog)
  - [ ] provider fallback info, mark old providers
...
- [ ] - End-to-End encryption Part 3:
  ...
  - [ ] encrypt locally stored keys https://developer.mozilla.org/en-US/docs/Web/API/SubtleCrypto/wrapKey and allowing a user master password to decrypt those keys
- [ ] consider https://docs.codeberg.org/codeberg-pages/
- [ ] share location
- [ ] Marketplace (DFX Implementation - Sponsoring?)
  - [ ] integrate https://github.com/webxdc - https://webxdc.org/
...
- [x] add dark mode (@media (prefers-color-scheme: dark))
...
- [ ] local, private client side AI eg. https://github.com/ngxson/wllama, https://medium.com/@heyamit10/fine-tuning-gguf-models-a-practical-guide-d4cc83f9e157, https://technotim.live/posts/ai-stack-tutorial/, https://www.kla.tv/36858; https://www.lewrockwell.com/2025/11/mark-keenan/the-matrix-is-talking-to-the-matrix-how-ai-is-replacing-human-thought/
  - [ ] Do: https://www.udemy.com/course/hugging-face-transformersjs-free-ai-models-for-javascript/learn/lecture/39875390?start=0#overview
...
- [ ] Hint dialog component (help and texts with translation)
  ...
  - [ ] tour explaining the website, self made or with something like https://developer.mozilla.org/en-US/docs/Web/API/Popover_API or https://driverjs.com/
...
- [/] https://www.runonflux.io/ done for website 8080 interface (not applicable since it load balances between a minimum of three instances, which makes it unreliable if clients land on different nodes aka. instances.)
  - [ ] web docker container, run some through pangolin
  - [ ] video call in iframe from local instance (https://jitsi.github.io/handbook/docs/dev-guide/dev-guide-ljm-api), jitsi deep fe integration. user video attached to chat on top. multiple jitsi instances support.
  - [x] https://github.com/localtunnel/server
  - [x] website on flux
  - [x] forward proxy to local instance https://pangolin.fossorial.io (evtl. local tunnel) (https://github.com/anderspitman/awesome-tunneling expl. https://github.com/rapiz1/rathole)
...
- [ ] native operating system share dialog behavior (https://docs.pwabuilder.com/#/builder/manifest?id=share_target-object)
  - [ ] open pwa on decentral.ninja link DCN:... link (double point)
